...
class ConfirmDeleteUserDialog(ctk.CTkToplevel):
    def __init__(self, parent, controller, user, *args, **kwargs):
        ...
        # Create button YES
        self.yes_button = ctk.CTkButton(self, text="Yes", command=lambda: self.controller.delete_user_event(user=user, input=True, dialog=self))
        self.yes_button.grid(row=1, column=0, padx=20, pady=20, sticky="nsew")

        # Create button NO
        self.no_button = ctk.CTkButton(self, text="Cancel", command=lambda: self.controller.delete_user_event(user=user, input=False, dialog=self))
