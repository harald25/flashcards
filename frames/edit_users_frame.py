    ...
    def add_user_buttons(self):
        ...
        for i in range(len(self.controller.model.all_users) + 1):
            if i < len(self.controller.model.all_users):
                ...
                edit_user_button = ctk.CTkButton(
                    self,
                    text="Edit",
                    width=70,
                    command=lambda user=user: self.controller.edit_user_event(user),
                )
                edit_user_button.grid(
                    row=row, column=1, padx=0, pady=(0, 10), sticky="e"
                )
                ...
                delete_user_button = ctk.CTkButton(
                    ...
                    text="Delete",
                    width=70,
                    command=lambda user=user: self.controller.open_confirm_delete_user_dialog(
                        user
                    ),
                )
                delete_user_button.grid(
                    row=row, column=2, padx=(5, 0), pady=(0, 10), sticky="e"
                )
            else:
                ...
                edit_user_button = ctk.CTkButton(
                    self, text="Add", width=70, command=self.controller.add_user_event
                )
                edit_user_button.grid(
                    row=0, column=1, columnspan=2, padx=0, pady=(0, 10), sticky="e"
                )
