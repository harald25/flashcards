...
class PlayFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.header_label = ctk.CTkLabel(
            self.headings_frame, text="F L A S H C A R D S", font=ctk.CTkFont(size=24)
        )
        self.header_label.grid(row=1, column=0, padx=0, pady=0, sticky="ew")

        self.post_header_label = ctk.CTkLabel(
            self.headings_frame, text="Study like the champs!", text_color="gray"
        )
        ...
        self.correct_button_label.grid(
            row=0, column=0, columnspan=3, sticky="nsew", padx=20, pady=(10, 0)
        )

        self.correct_segbuttons = ctk.CTkSegmentedButton(
            self.buttons_frame, width=260, dynamic_resizing=False
        )
        self.correct_segbuttons.grid(
            row=1, column=0, columnspan=3, padx=20, pady=(0, 20)
        )
        self.correct_segbuttons.configure(values=["Wrong", "Skip", "Correct"])
        self.correct_segbuttons.set("Skip")

        self.next_user_label = ctk.CTkLabel(
            self.buttons_frame,
            text=f"The next to answer is {self.controller.model.active_users[0]}.",
            text_color="gray",
        )
        self.next_user_label.grid(
            row=2, column=0, columnspan=3, sticky="nsew", padx=20, pady=(0, 0)
        )
        ...
        next_button = ctk.CTkButton(
            ...
            command=controller.next_card_button_event,
        )
        ...
        self.randomness_label_mid = ctk.CTkLabel(
            self.buttons_frame,
            text=f"Degree of randomness: {50}%",
            text_color="gray",
            width=200,
        )
