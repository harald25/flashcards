...
class SettingsFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # Configure variables
        self.open_confirm_delete_dialog_exists = False

        # Configure grid layout
        self.grid_rowconfigure((0, 1), weight=1)
        self.grid_columnconfigure((0, 1), weight=1)

        # Create scrollable frame participants
        self.scrollable_frame_participants = ctk.CTkScrollableFrame(self, label_text="Choose participants")
        self.scrollable_frame_participants.grid(row=0, column=0, padx=(40, 20), pady=(20, 20), sticky="nsew")
        self.scrollable_frame_participants.grid_columnconfigure(0, weight=1)
        self.participants_switches = {}
        self.participants_variables = {}

        # # Create participants switches
        # for i in range(len(flashcards.all_users)):
        #     user = flashcards.all_users[i]
        #     self.participants_variables[user] = ctk.StringVar(self, f"{user}/0")
        #     switch = ctk.CTkSwitch(self.scrollable_frame_participants, text=f"{user}", state="on", offvalue=f"{user}/x", onvalue=f"{user}/1", variable=self.participants_variables[user], command=self.update_participating_users)
        ...
        # TODO
        # for i in range(len(flashcards.all_topics)):
        #     topic = flashcards.all_topics[i]
        ...
        # Add user buttons TODO
        # self.edit_user_buttons = {}
        #
        # for i in range(len(flashcards.all_users) + 1):
        #     if i < len(flashcards.all_users):
        #         user = flashcards.all_users[i]
        #         row = i+1
        #
        ...
        # TODO
        # restore_data_date_label = ctk.CTkLabel(self.configure_data_frame, text=f"{flashcards.get_time_last_backup()}", justify=ctk.LEFT, anchor="w")
        # restore_data_date_label.grid(row=2, column=0, padx=10, pady=(0, 10), sticky="w")
        #
        # restore_data_date_button = ctk.CTkButton(self.configure_data_frame, text="Restore", width=70, command=flashcards.restore_from_backup)
