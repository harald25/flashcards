...
class SettingsFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.scrollable_frame_active_users = ChooseActiveUserFrame(
            self, self.controller
        )
        self.scrollable_frame_active_users.grid(
            row=0, column=0, padx=(40, 20), pady=(20, 20), sticky="nsew"
        )

        # Create scrollable frame for choosing active topics
        self.scrollable_frame_active_topics = ChooseActiveTopicsFrame(
            self, self.controller
        )
        self.scrollable_frame_active_topics.grid(
            row=0, column=1, padx=(20, 40), pady=(20, 20), sticky="nsew"
        )

        # Create scrollable frame for editing users
        self.scrollable_frame_edit_users = EditUsersFrame(self, self.controller)
        self.scrollable_frame_edit_users.grid(
            row=1, column=0, padx=(40, 20), pady=(0, 20), sticky="nsew"
        )

        # Configure data frame
        self.configure_data_frame = BackupsFrame(self, self.controller)
        self.configure_data_frame.grid(
            row=1, column=1, padx=(20, 40), pady=(0, 20), sticky="nsew"
        )
