...
class Controller:
    ...
    def update_play_frame(self, card_preview=False):
        ...
        self.view.play_frame.next_user_label.configure(
            text=f"The next to answer is {self.model.active_users[0]}."
        )
    ...
    def update_list_all_cards_frame(self, topic="All topics"):
        ...
        self.view.edit_cards_frame.list_all_cards_frame = ListAllCardsFrame(
            parent=self.view.edit_cards_frame, controller=self, topic=topic
        )
        self.view.edit_cards_frame.list_all_cards_frame.grid(
    ...
    def update_edit_cards_frame(self):
        ...
        self.view.edit_cards_frame.grid(
    ...
    def randomness_slider_event(self, value):
        ...
        # Update label
        self.view.play_frame.randomness_label_mid.configure(
            text=f"Degree of randomness: {value}%"
        )
    ...
    def active_user_switch_event(self, user):
        ...
        for (
            ...
            self.view.settings_frame.scrollable_frame_active_users.active_users_switches
        ):
            active_users_counter += self.view.settings_frame.scrollable_frame_active_users.active_users_switches[
                key
            ].get()
    ...
    def active_topic_switch_event(self, topic):
        ...
        for (
            ...
            self.view.settings_frame.scrollable_frame_active_topics.active_topics_switches
        ):
            active_topics_counter += self.view.settings_frame.scrollable_frame_active_topics.active_topics_switches[
                key
            ].get()
    ...
    def edit_user_event(self, user):
        dialog = ctk.CTkInputDialog(
            text=f"What is the new name of {user}?", title="Edit user"
        )
    ...
    def add_user_event(self):
        dialog = ctk.CTkInputDialog(
            text=f"What is the new name of the new user?", title="Add user"
        )
    ...
    def confirm_save_backup_event(self):
        ...
        if len(list_of_files) >= 10:
            confirm_delete_oldest_backup_dialog = ConfirmDeleteOldestBackupDialog(
                parent=self.view, controller=self
            )
    ...
    def delete_backup_event(self, backup):
        # Open confirm dialog
        dialog = ConfirmDeleteBackupDialog(
            parent=self.view, controller=self, backup=backup
        )
        dialog.wm_transient(self.view)
    ...
    def restore_from_backup_event(self, backup):
        # Open confirm dialog
        dialog = ConfirmRestoreFromBackupDialog(
            parent=self.view, controller=self, backup=backup
        )
        dialog.wm_transient(self.view)
    ...
    def edit_card_text_event(self, old_card_text):
        print(old_card_text)
        dialog = EditCardCialog(
            parent=self.view, controller=self, old_card_text=old_card_text
        )
        dialog.wm_transient(self.view)
    ...
    def edit_card_text_dialog_event(self, dialog):
        ...
        if (
            self.model.check_if_new_card_text_is_valid(old_card_text, new_card_text)
            is True
        ):
            # Destroy dialog window
            dialog.destroy()
    ...
    def delete_card_event(self, card_text):
        dialog = ConfirmDeleteCardDialog(
            parent=self.view, controller=self, card_text=card_text
        )
        dialog.wm_transient(self.view)
